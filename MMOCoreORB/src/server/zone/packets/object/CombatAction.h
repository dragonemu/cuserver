...
public:
	...
	CombatAction(CreatureObject* attacker, CreatureObject* defender, uint32 commandcrc, uint32 animcrc, uint8 hit, uint8 trails, int damage = 0, long weaponID = 0) :
	...
	CombatAction(CreatureObject* attacker, TangibleObject* defender, uint32 commandcrc, uint32 animcrc, uint8 hit, uint8 trails, int damage = 0, long weaponID = 0) :
				ObjectControllerMessage(attacker->getObjectID(), 0x1B, 0xCC) {
		...
		insertByte(0x00); // DefenderEndPosture: 0x0-Standing 0x1-Kneeling 0x2-Prone 0xD-Incapacitated
		insertByte(hit); // HitType: 0x0-MISS 0x1-HIT 0x2-BLOCK 0x3-DODGE 0x5-COUNTER 0x7-RICOCHET 0x8-REFLECT 0x9-REFLECT_TO_TARGET
		insertByte(0x00); // DefenderSpecialMoveEffect: disabled in the client
	...
	CombatAction(TangibleObject* attacker, CreatureObject* defender, uint32 commandcrc, uint32 animcrc, uint8 hit, uint8 trails, int damage = 0, long weaponID = 0) :
		ObjectControllerMessage(attacker->getObjectID(), 0x1B, 0xCC) {
		...
		insertByte(0); // AttackerEndPosture: 0x0-Standing 0x1-Kneeling 0x2-Prone 0xD-Incapacitated
		insertByte(trails); // Trails: 0x01-left_foot 0x02-right_foot 0x04-left_hand 0x08-right_hand 0x10-weapon
		insertByte(0x00); // AttackerSpecialMoveEffect: disabled in the client
