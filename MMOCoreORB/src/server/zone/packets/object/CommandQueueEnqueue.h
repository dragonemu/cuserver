...
#include "server/zone/objects/player/events/CommandWarmUpTask.h"
...
public:
	...
	void run() {
		...
		if (commandCount >= 2 && miliDifference < 1000)
			...
			if (!player->checkCooldownRecovery("command_" + (!queueCommand->getCoolDownGroup().isEmpty() ? queueCommand->getCoolDownGroup() : "0"))) {
				player->clearQueueAction(actionCount, 0, 0, 0, actionCRC);

				return;
			}

			// Dreadlow - Will warm up even when incapped, may need to move the below to sometime after the onFail checks
			//			- Or call the onFail checks now
			if (queueCommand->getWarmUp() > 0) {
				BaseMessage* warmupStart = new CommandTimer(player, actionCount, actionCRC, queueCommand->getWarmUp());
				player->sendMessage(warmupStart);

				Reference<CommandWarmUpTask*> cwTask = new CommandWarmUpTask(player, actionCRC, actionCount, targetID, arguments);
				player->addPendingTask("commandWarmUpTask", cwTask, queueCommand->getWarmUp() * 1000);
			} else
