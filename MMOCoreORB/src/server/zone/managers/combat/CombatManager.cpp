...
int CombatManager::doTargetCombatAction(CreatureObject* attacker, WeaponObject* weapon, TangibleObject* tano, const CreatureAttackData& data) {
	...
	if (tano->isCreatureObject()) {
		...
		broadcastCombatAction(attacker, tano, weapon, data, 0x01, damage);
...
int CombatManager::doTargetCombatAction(CreatureObject* attacker, WeaponObject* weapon, CreatureObject* defender, const CreatureAttackData& data) {
	...
	if (data.isStateOnlyAttack()) {
		broadcastCombatAction(attacker, defender, weapon, data, hitVal, damage);
		return 0;
	}
...
int CombatManager::doTargetCombatAction(TangibleObject* attacker, WeaponObject* weapon, CreatureObject* defenderObject, const CreatureAttackData& data) {
	...
	combatAction = new CombatAction(attacker, defenderObject, commandCRC, animationCRC, hitVal, CombatManager::DEFAULTTRAIL);
...
int CombatManager::getDefenderDefenseModifier(CreatureObject* defender, WeaponObject* weapon, TangibleObject* attacker) {
	...
	if (targetDefense > 125)
		targetDefense = 125.f + ((targetDefense - 125)  * .25);
...
int CombatManager::getArmorReduction(TangibleObject* attacker, WeaponObject* weapon, CreatureObject* defender, float damage, int poolToDamage, const CreatureAttackData& data){
	...
	if (data.getAttackType() == CombatManager::FORCEATTACK) {
		...
		int forceDefense = (defender->getSkillMod("rank_defense") / 10.f);
			forceShield += forceDefense;
		...
		if (forceFeedback > 0)
			attacker->inflictDamage(defender, System::random(2) * 3, System::random(100), true);
		...
		if (defender->getSkillMod("force_absorb") > 0 && defender->isPlayerCreature()) {
			...
			if (forcePower >= forcePowerMax)
				playerObject->setForcePower(forcePowerMax);
		...
		int forceDefense = (defender->getSkillMod("rank_defense") / 10.f);
			forceArmor += forceDefense;

		if (forceArmor > 0)

			defender->notifyObservers(ObserverEventType::FORCEBUFFHITARMOR, attacker, rawDamage - (damage *= 1.f - (forceArmor / 100.f)));

		if (bhShields > 0)

			defender->notifyObservers(ObserverEventType::SHIELDSHIT, attacker, rawDamage - (damage *= 1.f - (bhShields / 100.f)));
	}
	...
	if (armor != NULL && !armor->isVulnerable(weapon->getDamageType())) {
		...
		float armorBreak = (defender->getSkillMod("armor_break") / 100.f);
		/*if(defender->containsPendingTask("armor_break")){
			armorReduction = armorReduction * .75;
		}*/

		if(armorBreak > 0){
			armorReduction = armorReduction * armorBreak;
		}
...
float CombatManager::calculateDamage(CreatureObject* attacker, WeaponObject* weapon, CreatureObject* defender, const CreatureAttackData& data, Vector<int>& foodMitigation) {
	...
	if (data.getDamage() > 0) { // this is a special attack (force, heavy weapon, etc)
		int forceDamage = (attacker->getSkillMod("general_force_damage") / 100.f);
		int rankDamage = (attacker->getSkillMod("rank_offence") / 100.f);

		damage = data.getDamage()  + data.getDamage() * forceDamage;
		damage -= System::random(damage / 4);
		damage +=  damage * rankDamage;
		//damage += (weapon->getMaxDamage() - System::random(weapon->getMinDamage()));
	...
	if(exposure > 0){
		if(System::random(100) < exposure)
		damage *= 1.5;
	}

	double fortify = (defender->getSkillMod("fortify") / 100.f);
...
float CombatManager::calculateWeaponAttackSpeed(CreatureObject* attacker, WeaponObject* weapon, float skillSpeedRatio) {
	...
	float attackSpeed = ((skillSpeedRatio - ((float) speedMod / 100.0f)) + (weapon->getAttackSpeed() - ((float) speedMod / 100.0f)));

	if (jediSpeed > 0)
		attackSpeed = attackSpeed - jediSpeed;
	info("speedMod total: " + String::valueOf(speedMod), true);
...
bool CombatManager::applySpecialAttackCost(CreatureObject* attacker, WeaponObject* weapon, const CreatureAttackData& data) {
	...
	float mind = weapon->getActionAttackCost() * data.getMindCostMultiplier();
...
void CombatManager::broadcastCombatSpam(TangibleObject* attacker, TangibleObject* defender, TangibleObject* item, int damage, const String& file, const String& stringName, byte color) {
	...
	for (int i = 0; i < closeObjects.size(); ++i) {
		...
		if (object->isPlayerCreature() && attacker->isInRange(object, 70)) {
			...
			//CombatSpam* spam = new CombatSpam(attacker, defender, weapon, damage, "cmd_n", stringid, receiver);
			//receiver->sendMessage(spam);
		}
	}
}

void CombatManager::broadcastCombatAction(CreatureObject * attacker, TangibleObject * defenderObject, WeaponObject* weapon, const CreatureAttackData & data, uint8 hit, int damage) {
	CombatAction* combatAction = NULL;
	...
	if (defenderObject->isCreatureObject())
		combatAction = new CombatAction(attacker, defenderObject->asCreatureObject(), commandCRC, animationCRC, hit, data.getTrails(), damage, weapon->getObjectID());
	else
	...
	String targetEffect = data.getCommand()->getTargetEffect();
