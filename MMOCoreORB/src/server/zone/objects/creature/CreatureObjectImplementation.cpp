...
void CreatureObjectImplementation::clearQueueAction(uint32 actioncntr, float timer, uint32 tab1, uint32 tab2, uint32 actionCRC) {
	if (!isPlayerCreature())
		return;
...
void CreatureObjectImplementation::setEncumbrance(int type, int value,
	...
	if (encumbrances.get(type) == value)
		return;
	//TODO Remove? Fix? Change? I think change.
...
void CreatureObjectImplementation::addBuff(uint32 crc) {
	creatureBuffs.addBuff(crc, asCreatureObject(), 0);
}
...
void CreatureObjectImplementation::removeBuff(uint32 crc) {
	creatureBuffs.removeBuff(crc, asCreatureObject());
}

void CreatureObjectImplementation::clearBuffs(bool updateclient) {
	creatureBuffs.clearBuffs(asCreatureObject());
}
...
void CreatureObjectImplementation::forceCloakRemoval() {
	if (hasBuff(String("forcecloak").hashCode()))
		removeBuff(String("forcecloak").hashCode());
	...
	//broadcastMessage(removeCloakEffectOther, false);

	playEffect("clienteffect/combat_special_attacker_cover.cef");
	showFlyText("combat_effects", "no_cloak", 1.0, 255, 0, 0);

	sendSystemMessage("@jedi_spam:remove_forcecloak");
}
